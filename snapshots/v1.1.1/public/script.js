...
async function loadSharedAppData() {
  const serverData = normalizeAppData(await requestJson(`${apiBase}/app-data`));
...
async function saveSharedSection(endpoint, key, value) {
...
function createOrderHistoryRecord({ selectedUser, orderState, items }) {
  const orderItems = buildDishSummaryData(items).map((dish) => ({
    id: dish.id,
    name: dish.name,
    count: dish.count,
  }));
  ...
  return {
    ...
    orderDate: orderState.selectedDate,
    completed: false,
...
async function initializeHomePage() {
  ...
  async function saveOrderState() {
    await saveSharedSection("order-state", "orderState", orderState);
  }
  ...
  homeOrderForm.addEventListener("submit", async (event) => {
    ...
    try {
      const orderHistory = appData.orderHistory;
      ...
      await saveSharedSection("order-history", "orderHistory", orderHistory);
      orderState.items = [];
      await saveOrderState();
      showHomeOrderMessage(`已为 ${selectedUser.name} 建立一笔本地订单。`);
      homeOrderForm.reset();
      renderUserOptions();
      renderSelectedDishes();
    } catch (error) {
...
async function initializeDishesPage() {
  ...
  async function saveDishes() {
    dishes = await saveSharedSection("dishes", "dishes", dishes);
  }
  ...
  form.addEventListener("submit", async (event) => {
    ...
    try {
      ...
      dishes = dishes.filter((dish) => dish.id !== newDish.id);
      showMessage("新增菜品失败，请刷新后重试。");
    }
  });
...
async function initializeUsersPage() {
  ...
  async function saveUsers() {
    users = await saveSharedSection("users", "users", users);
  }
...
async function initializeOrdersPage() {
  ...
  const appData = await loadSharedAppData();
  let orderHistory = appData.orderHistory;

  async function saveOrderHistory() {
    orderHistory = await saveSharedSection("order-history", "orderHistory", orderHistory);
  }
  ...
  pendingOrderList.addEventListener("click", async (event) => {
    ...
    orderHistory = orderHistory.map((order) =>
      order.orderDate === selectedDate ? { ...order, completed: true } : order
    );
